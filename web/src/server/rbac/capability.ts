/**
 * A-08 (web/server port) — Capability matrix for Next.js server layer.
 * CAPABILITY_MATRIX copied verbatim from backend/src/rbac/capability.enum.ts.
 * Only the import line differs (types from @furama/shared).
 *
 ...
 * `roleHasCapability(role, capability) === true`; scope-aware checks live in server actions.
 */
...
/** Plain lookup — `true | 'scope' | false`. Use server actions for scope resolution. */
